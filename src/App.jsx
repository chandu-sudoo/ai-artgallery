    ...
    <>
      ...
      {activeArtwork && (
        <div style={{
          ...
          <div style={{ 
            backgroundColor: '#fff', padding: 40, borderRadius: 16, maxWidth: 800, width: '90%',
            boxShadow: '0 20px 40px rgba(0,0,0,0.1)', display: 'flex', gap: '40px',
            fontFamily: "'Inter', sans-serif"
          }}>
            {activeArtwork.isEmpty ? (
              <div style={{ 
                width: '400px', height: '400px', backgroundColor: '#eee', 
                border: '4px dashed #ccc', borderRadius: '8px', flexShrink: 0,
                display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#999'
              }}>
                <h3>Reserved for future artwork</h3>
              </div>
            ) : activeArtwork.imagePath ? (
              <div style={{ 
                width: '400px', height: '400px', flexShrink: 0,
                display: 'flex', justifyContent: 'center', alignItems: 'center',
                backgroundColor: '#f5f5f5', borderRadius: '8px', overflow: 'hidden',
                boxShadow: 'inset 0 0 20px rgba(0,0,0,0.1)'
              }}>
                {/* We use an img tag to handle missing image gracefully with alt text until they upload */}
                <img 
                  ...
                  onError={(e) => {
                    ...
                    e.target.parentElement.innerHTML = `<span style="color:white; font-weight:bold;">${activeArtwork.title} (Image Pending Upload)</span>`;
                  }}
                />
              </div>
            ) : (
              <div style={{ 
                width: '400px', height: '400px', backgroundColor: activeArtwork.color, 
                borderRadius: '8px', flexShrink: 0,
                boxShadow: 'inset 0 0 20px rgba(0,0,0,0.2)'
              }} />
            )}
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <h1 style={{ margin: '0 0 10px 0', fontSize: '2.5rem', fontWeight: 800 }}>{activeArtwork.title}</h1>
